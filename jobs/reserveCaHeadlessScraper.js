...
module.exports = async function headlessScraper(campsiteFinder) {
  ...
  while (true) {
    ...
    const currentAvailabilities = await page.evaluate(() =>
      [...document.querySelectorAll('.blue_brd_box')].reduce((avails, elem) => {
  ...
  const availabilities = allDates
    .map(requestedDatesArr => {
      const matchingUnits = Object.keys(availableDatesBySiteId).filter(siteId =>
        hasAllRequestedDates(requestedDatesArr, availableDatesBySiteId[siteId])
      );
